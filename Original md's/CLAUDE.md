...
- **Working Title:** *Jim's Boba Shop* (placeholder — Sloane can rename)
- **Protagonist(s):** The player runs the shop. **Jim** (scary demon in an old boba tee shirt and jeans) is the recurring presence — possibly an employee, possibly a customer, possibly something worse. His exact role is the hook to develop in chat with Sloane.
- **Setting / Opening Shot:** The player starts **outside the shop**, looking at an empty storefront. They build the shop first — counter, boba machines, tables, decor — then open for customers. The build phase and the run phase are both core gameplay.
- **Core Verb:** prep food and drinks for customers, take coins, decorate and expand the shop. Cozy shop-sim moment-to-moment.
- **The "DAD LOOK!" Moment:** a jumpscare about to happen. The shop runs cute and cozy ~95% of the time — then something shifts (lights flicker, a customer's face goes wrong, Jim walks in) and the player has a half-second to feel it coming before it lands. The build-up is the moment she wants to point at.
- **Art Direction:** soft palette anchored on strawberry pink / matcha green / powder blue, with horror accents (deep shadow, sickly green, blood-boba red) layered in for the scare beats. Kawaii horror — cute window, wrong reflection. Jim drawn procedurally on canvas: demon silhouette with the human-mundane clothes (boba tee, jeans) — the contrast is the character.
- **Sound Design:**
  - **Cozy ambient layer (always running while shop is open):** lo-fi background music, boba shaker rhythm tied to the prep action, ice clinking, coin jingles on every payment, soft customer "thank you" beeps when an order completes.
  - **Tension layer (fades in before scares):** low hum that slowly builds underneath the cozy layer, then the lo-fi music suddenly drops out — silence is the cue. Optional weird whisper or heartbeat for the worst beats.
  - **Scare hit:** sharp BAM with screen shake when the moment lands (Sloane explicitly wants the scare *itself*, not just the build-up — the payoff matters).
  - **Sound returns to cozy:** after the beat passes, lo-fi music fades back in. The contrast is what makes both layers feel right.
- **Non-Negotiables for Game One:**
  - Build phase before the shop opens.
  - A coin economy with visible unlocks.
  - At least one real jumpscare moment with proper build-up.
  - Never-ending structure — no "you win" screen.
  - Jim shows up.
  - Both sound layers (cozy + tension) are present from day one — even if the cozy layer is just lo-fi + shaker and the tension layer is just a hum + silence cue. Sound ships in v1, not v2.
