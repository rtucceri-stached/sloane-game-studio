...
export class FoodZone {
  constructor(canvas) {
    ...
    this.balloon = {
      ...
      bobPhase: 0,
      shadowOffsetX: -50,
      shadowOffsetY: 12, // wrong drop-shadow detail
    };
  ...
  _drawCoaster(cx, cy, w, h, t) {
    ...
    for (let x = -w / 2; x <= w / 2; x += 6) {
      const localT = (x + w / 2) / w;
      const y =
        -Math.sin(localT * Math.PI * 1.8) * h * 0.5 - Math.sin(localT * Math.PI * 4) * h * 0.18 + 6;
      if (x === -w / 2) ctx.moveTo(x, y);
      else ctx.lineTo(x, y);
    }
    ctx.stroke();
  ...
  _drawMidSilhouettes(t) {
    ...
    for (const s of this.midSilhouettes) {
      ...
      ctx.fillStyle = PAL.rideMid;
      this._roundRect(wx, wy, s.w, s.h, 4);
      ctx.fill();
  ...
  _drawLanternPosts(t) {
    ...
    for (const [i, j] of this.stringLightLines) {
      const a = this.lanternPosts[i],
        b = this.lanternPosts[j];
      if (!a || !b) continue;
      const ax = a.x - this.camera.x,
        ay = a.y - this.camera.y - a.h + 6;
      const bx = b.x - this.camera.x,
        by = b.y - this.camera.y - b.h + 6;
  ...
  _drawBalloon(t) {
    ...
    ctx.fillStyle = 'rgba(0,0,0,0.4)';
    ctx.beginPath();
    ctx.ellipse(wx + b.shadowOffsetX, wy + b.shadowOffsetY + 280, 18, 5, 0, 0, Math.PI * 2);
    ctx.fill();
  ...
  _drawDustMotes() {
    ...
    for (const d of this.dustMotes) {
      const wx = d.x - this.camera.x,
        wy = d.y - this.camera.y;
